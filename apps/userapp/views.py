    ...
    def get(self, request):
        try:
            if request.session['user']['identity'] == 'student':
                ...
                es_score = ExamStudentScore.objects.filter(student_id=stu)
                ...
                all_exam = Exam.objects.filter(teacher_id=teacher).all()
                my_exam = Exam.objects.filter(teacher_id=teacher).all()
                ...
                all_exam_student_details = []
                for specialty in specialtys:
                    all_exam_student_details.append(
                        StudentProfile.objects.filter(specialty_id=specialty.specialty_id).all())  # 该教师教授所有专业所有学生学习情况


                all_exam_student_score = ExamStudentScore.objects.all()
                examstudent_list = []
                for student in all_exam_student_details:
                    for student2 in student:
                        s = []
                        l = []
                        one_time = FileCheck.timeConvert(self, int(student2.total_time))
    ...
    def post(self, request):
        ...
        try:
            # if code == '123456':
                ...
                if teacher:
                    return JsonResponse({'status': 'error'})  # 验证正确但学号存在
                else:
                    # 不存在新建
